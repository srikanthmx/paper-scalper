...
def main() -> None:
    ...
    for name in STRATEGIES:
        _report(journal, name)
    _report(journal, None)
    journal.close()


def _report(journal: Journal, strategy: str | None) -> None:
    s = journal.summary(strategy)
    trades = journal.trades(limit=100_000, strategy=strategy)
    print(f"\n## {'Combined' if strategy is None else strategy}\n")
    pf = s["profit_factor"]
    ...
    if trades:
        print("\n### Last 10 trades\n")
        print("| exit (UTC) | strategy | side | entry | exit | net $ | net % | exit reason |")
        print("|---|---|---|---|---|---|---|---|")
        for t in trades[:10]:
            print(f"| {_ts(t['exit_ts'])} | {t['strategy']} | {t['side']} "
                  f"| {t['entry_price']:.2f} | {t['exit_price']:.2f} | {t['net_pnl']:.2f} "
